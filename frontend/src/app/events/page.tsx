...
export default function EventsPage() {
  ...
  const getEventTypeColor = (type: string, isActive: boolean = false) => {
    ...
    const colorSet = colors[type] || {
      ...
      inactive: "bg-muted/30 text-muted-foreground/50 border border-border/30",
    };
              ...
              {eventTypes.map((type) => {
                ...
                return (
                  <button
                    ...
                    className={`min-h-[44px] min-w-[44px] px-4 py-2 rounded text-sm font-semibold uppercase tracking-wider transition-all duration-200 ease-in-out hover:scale-105 inline-flex items-center justify-center ${getEventTypeColor(
                      type.value,
                      isActive
                    )}`}
