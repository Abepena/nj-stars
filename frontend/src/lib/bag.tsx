...
export function BagProvider({ children }: { children: React.ReactNode }) {
  ...
  const selectAllItems = useCallback(() => {
    ...
    setSelectedItems(
      new Set(
        bag.items.filter((item) => item.is_available).map((item) => item.id)
      )
    );
  }, [bag]);
  ...
  const addToBag = useCallback(
    async (
      ...
      try {
        ...
        // Open the bag drawer to show the user what was added
        openBag();
      } catch (err) {
        const message =
          err instanceof Error ? err.message : "Failed to add to bag";
        setError(message);
        throw err;
      }
    },
    ...
    async (itemId: number, quantity: number) => {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/api/payments/bag/items/${itemId}/`,
          {
            method: "PATCH",
            headers: getHeaders(),
  ...
  const clearBag = useCallback(async () => {
    try {
      ...
      // Reset bag state
      setBag((prev) =>
        prev ? { ...prev, items: [], item_count: 0, subtotal: "0.00" } : null
      );
    } catch (err) {
      const message =
        err instanceof Error ? err.message : "Failed to clear bag";
      setError(message);
      throw err;
    }
  }, [getHeaders]);
  ...
  const checkout = useCallback(
    async (
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/payments/checkout/bag/`,
        {
          method: "POST",
          headers: getHeaders(),
          body: JSON.stringify({
      ...
      if (typeof window !== "undefined") {
        localStorage.setItem(
          CHECKOUT_ITEMS_KEY,
          JSON.stringify(checkoutItemIds)
        );
      }
